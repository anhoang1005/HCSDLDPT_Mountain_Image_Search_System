...
def convert_to_hog_hsv(img_path):
    ...
    combined_features = np.concatenate((fd, hist_hsv.flatten()))
...
for img_file in image_files:
    ...
    combined_features_list.append({'image_name': img_file, 'hog_hsv_features': convert_to_hog_hsv(img_path)})
    print(dem, end=' ')
    print("Trích rút thành công!\n")
    dem = dem + 1

np.save("DACTRUNG/hog_hsv_features.npy", combined_features_list)
